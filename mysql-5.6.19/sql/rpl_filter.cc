...
int 
Rpl_filter::add_do_table_hash(const char* table_spec) 
{
  DBUG_ENTER("Rpl_filter::add_do_table");
  if (!do_table_hash_inited)
    init_table_rule_hash(&do_table_hash, &do_table_hash_inited);
  table_rules_on= 1;
  DBUG_RETURN(add_table_rule_to_hash(&do_table_hash, table_spec, (int)strlen(table_spec)));
}
...
Rpl_filter::set_do_table(const char* table_spec)
{
  ...
  if (do_table_hash_inited)
    my_hash_reset(&do_table_hash);

  status= parse_filter_rule(table_spec, &Rpl_filter::add_do_table_hash);
...
int 
Rpl_filter::add_ignore_table_hash(const char* table_spec) 
{
  DBUG_ENTER("Rpl_filter::add_do_table");
  if (!ignore_table_hash_inited)
    init_table_rule_hash(&ignore_table_hash, &ignore_table_hash_inited);
  table_rules_on= 1;
  DBUG_RETURN(add_table_rule_to_hash(&ignore_table_hash, table_spec, (int)strlen(table_spec)));
}
...
Rpl_filter::set_ignore_table(const char* table_spec)
{
  ...
  if (ignore_table_hash_inited)
    my_hash_reset(&ignore_table_hash);

  status= parse_filter_rule(table_spec, &Rpl_filter::add_ignore_table_hash);
