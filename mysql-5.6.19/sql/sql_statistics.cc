...
static const char *sql_command[] = {
  "SQLCOM_SELECT", "SQLCOM_CREATE_TABLE", "SQLCOM_CREATE_INDEX", "SQLCOM_ALTER_TABLE", "SQLCOM_UPDATE", "SQLCOM_INSERT", "SQLCOM_INSERT_SELECT",
...
struct {
  ...
  ulong         discard_because_too_long;
  ulong         output_counts;
  ulong         clean_counts;
  ulong         oos_sql_counts; /* out of size sql counts*/
...
bool update_exclude_db_list()
{
  ...
  {
    char *temp = my_strdup(statistics_exclude_db, MYF(MY_WME));
    char *token = NULL, *saveptr = NULL;
    for (token = strtok_r(temp, ";", &saveptr); token; token= strtok_r(NULL, ";", &saveptr))
    {
      char *name = my_strdup(token, MYF(MY_WME));
...
bool update_exclude_sql_list()
{
  ...
  {
    char *temp = my_strdup(statistics_exclude_sql, MYF(MY_WME));
    char *token = NULL, *saveptr = NULL;
    for (token = strtok_r(temp, ";", &saveptr); token; token= strtok_r(NULL, ";", &saveptr))
    {
      sql = my_strdup(token, MYF(MY_WME));
...
static void init_statistics_hash(void)
{
  ...
  (void)my_hash_init(&stat_table_hash, system_charset_info, 500,
    0, 0,(my_hash_get_key)get_key_tablestats, (my_hash_free_key)free_tablestats, 0);
#endif
}
...
static void compute_query_digest(char digest[], unsigned char *query, int length)
{
  ...
  array_to_hex((char *) digest, query, length);
}
...
int statistics_init(void)
{
  ...
  mysql_thread_create(0, &thread_id, NULL, statistics_output_thread, NULL);
  return 0;
}
